...
void atomic_wait(_Atomic(uint32_t)* ptr, uint32_t old) {
  return atomic_wait_explicit(ptr, old, __ATOMIC_SEQ_CST);
}

void atomic_notify_one(_Atomic(uint32_t)* ptr) {
  PlatformWakeOne((uint32_t*)ptr);
}

void atomic_notify_all(_Atomic(uint32_t)* ptr) {
